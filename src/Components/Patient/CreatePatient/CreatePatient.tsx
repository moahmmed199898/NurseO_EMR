...
import {$history, SaveButton, Button, StudentReport, SelectInput, Input, EmptyCard,
    Allergy, CustomOrder, Flag, MedicalIssue, MedicationOrder, Note, PatientChart, Time} from "nurse-o-core"
            ...
            <div className="grid">
                ...
                <EmptyCard title="Create Patient" className="block m-auto w-70vw" admin>
                        <form action='#' className="mx-28 py-5" ref={this.formRef} onSubmit={e=>e.preventDefault()}>
                            <Input id="id" onChange={e=>this.setState({id:e.currentTarget.value})} value={this.state.id}>Barcode ID</Input>
                            <Input id="name" onChange={e=>this.setState({name:e.currentTarget.value})} value={this.state.name}>Patient Name</Input>
                            <Input id="dob" type="date" onChange={this.onDateChangeHandler.bind(this)} value={this.state.dob}>Date of Birth</Input>
                            ...
                            <Input id="height" value={this.state.height} onChange={e=>this.setState({height:e.currentTarget.value})}>Height</Input>
                            <Input id="weight" value={this.state.weight} onChange={e=>this.setState({weight:e.currentTarget.value})}>Weight</Input>
                            <Input id="simTime" value={`${this.state.time.hour.toString().padStart(2,"0")}:${this.state.time.minutes.toString().padStart(2,"0")}`} type="time" onChange={this.onTimeChangeHandler.bind(this)}>Sim Time</Input>
                            <Input id="lab" value={this.state.labDocURL} type="url" notRequired onChange={e=>this.setState({labDocURL: e.currentTarget.value})}>Lab Document URL</Input>
                            ...
                            <div className='flex justify-center ml-40 mt-10'>
                                <Button>Clear</Button>
                                <Button onClick={this.onPreviewClickHandler.bind(this)} disabled className='cursor-not-allowed bg-gray-400'>Preview</Button>
